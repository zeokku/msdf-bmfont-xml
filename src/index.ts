...
function generateBMFont(fontPath, opt, callback) {
  ...
  const distanceRange = (opt.distanceRange = utils.valueQueue([
    opt.distanceRange,
    reuse.distanceRange,
    4,
  ]));
  const fontSize = (opt.fontSize = utils.valueQueue([opt.fontSize, reuse.fontSize, 42]));
  const fontSpacing = (opt.fontSpacing = utils.valueQueue([
    opt.fontSpacing,
    reuse.fontSpacing,
    [0, 0],
  ]));
  const pad = distanceRange >> 1;
  const fontPadding = (opt.fontPadding = utils.valueQueue([
    opt.fontPadding,
    reuse.fontPadding,
    [pad, pad, pad, pad],
  ]));
  const textureWidth = (opt.textureWidth = utils.valueQueue([
    opt.textureSize || reuse.textureSize,
    [512, 512],
  ])[0]);
  const textureHeight = (opt.textureHeight = utils.valueQueue([
    opt.textureSize || reuse.textureSize,
    [512, 512],
  ])[1]);
  const texturePadding = (opt.texturePadding = utils.valueQueue([
    opt.texturePadding,
    reuse.texturePadding,
    1,
  ]));
  ...
  const roundDecimal = (opt.roundDecimal = utils.valueQueue([
    opt.roundDecimal,
    reuse.roundDecimal,
  ...
  let charset = (opt.charset =
    (typeof opt.charset === "string" ? Array.from(opt.charset) : opt.charset) ||
    reuse.charset ||
    defaultCharset);
  ...
  const fontface =
    typeof fontPath === "string" ? path.basename(fontPath, path.extname(fontPath)) : filename;
        ...
        bin.rects.forEach((rect) => {
          if (rect.data.imageData) {
            ...
            if (debug) {
              ...
              const y_woffset =
                rect.y - rect.data.fontData.yoffset + baseline + (distanceRange >> 1);
              svg +=
                font
                  .charToGlyph(rect.data.fontData.char)
                  .getPath(x_woffset, y_woffset, fontSize)
                  .toSVG() + "\n";
            }
          }
